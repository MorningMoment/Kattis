...
int main(){
    ...
    for(int i=0;i<stu_num;i++){
        ...
        for(int j=0;j<ass_num;j++){
            double demo;
            cin>>demo;
            demo = (demo<5*(ass_num-1)+50)?demo:5*(ass_num-1)+50;
    ...
    //adjust scores
    double max_score = *max_element(total,total+stu_num);
    for(int i=0;i<stu_num;i++){
        int adjusted = ceil(total[i]*100/max_score);
        cout<<name[i]<<' '<<total[i]<<' '<<adjusted<<' '<<letter(adjusted)<<endl;
    }
